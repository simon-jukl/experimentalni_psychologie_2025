...
\displaylines{
response_i \sim Normal( \beta_0 + \beta_1 \cdot beat\_frequency + \beta_2 \cdot interval\_length + \beta_3 \cdot group + \\ 
\beta_4 \cdot beat\_frequency \cdot group  + \beta_5 \cdot beat\_frequency \cdot interval\_length, 
\sigma)
}
$$
...
bayesformula(
  ...
  response ~ beat_frequency + interval_length + group + beat_frequency:group + beat_frequency:interval_length

)
```


# Multilevel regression with (some) interactions
...
\beta_4 \cdot beat\_frequency \cdot group, 
\sigma)
}
$$
...
bayesformula(
  ...
  response ~ beat_frequency + interval_length + group + 
  beat_frequency:group + beat_frequency:interval_length +
