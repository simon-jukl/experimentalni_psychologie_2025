...
data_sim <- 
  trials |> 
  left_join(re, by = "participant") |> 
  mutate(
    mu = b_Intercept + re_intercept +
      (b_tempo + re_tempo) * tempo +
      (b_interval + re_interval) * interval_length +
      b_interaction * tempo * interval_length,
    sigma = sigma_Intercept + sigma_interval * interval_length,
...
data_sim |> 
  ...
  geom_smooth(aes(group = factor(tempo), color = tempo), method = "lm", se = FALSE) +
  geom_hline(yintercept = interval_levels, linetype = 2, color = "grey")


# model fit on data_sim
fit1_raw_sim <-       
  brm(
    formula = bf(
      response ~ 
    ...
    prior = c(
      ...
      set_prior("normal(0, 0.05)", class = "b", coef = "tempo"),
      set_prior("normal(1, 0.1)", class = "b", coef = "interval_length"),
      set_prior("normal(0, 0.05)", class = "b", coef = "tempo:interval_length"),
      set_prior("student_t(3, 0, 0.05)", class = "sd"),
      set_prior("student_t(3, 0, 0.15)", dpar = "sigma", lb = 0)
    ),
    cores = 4,
    chains = 4
  )

write_rds(fit1_raw_sim, here("data_simulation/fit1_raw_sim.rds"))


summary(fit1_raw)
pp_check(fit1_raw, ndraws = 100)
...
simulate_prior_predictive <- function(trials) {
  ...
  re <- tibble(
    ...
    sd_intercept = abs(rnorm(1, 0, 0.05)),
    sd_interval = abs(rnorm(1, 0, 0.05)),
    sd_tempo = abs(rnorm(1, 0, 0.05)),
  ...
  # sample sigma priors
  sd <- tibble(
    sigma_Intercept = rnorm(1, -2, 0.05),
    sigma_interval = rnorm(1, 0.2, 0.05)
  )
  ...
  trials |> 
    ...
    mutate(
      mu = b_Intercept + re_intercept +
        ...
        (b_tempo + re_tempo) * tempo +
        b_interaction * tempo * interval_length,
      sigma = exp(sigma_Intercept + sigma_interval * interval_length),
      response = rnorm(n(), mean = mu, sd = sigma)
    )
}
